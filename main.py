...
def get_lr_scheduler_params(args):
    ...
    elif lr_scheduler_params['type'] == 'none':
        return {}
...
def check_and_clean_dir(directory):
    if os.path.exists(directory):
        if not contain_alpha(os.path.basename(directory)):
...
def get_args():
    ...
    parser.add_argument('--lr_scheduler', type=str, choices=['exp', 'multi_step', 'none'],
                        default='exp')
    parser.add_argument('--gamma', type=float, default=0.99,
                        help='decay rate.')
    parser.add_argument('--milestones', type=list, default=[15, 50, 80],
