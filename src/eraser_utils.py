...
def load_documents(data_dir: str, docids: Set[str] = None) -> Dict[str, List[List[str]]]:
    ...
    for d in docids:
        with open(os.path.join(docs_dir, d), 'r', encoding='utf8') as inf:
...
def check_output_shape(model):
    '''
    使用hook函数检查前向传播模型输出形状
    '''
    import torch

    def output_shape(layer, _, out):
        template = '\t' * layer.__name__.count('.') + 'output: %s: %s'
        try:
            shape = out[0].shape if isinstance(out, (tuple, list)) else out.shape if isinstance(out, torch.Tensor) else out
        except:
            shape = out
        print(template % (layer.__name__, shape))

    def input_shape(layer, ipt):
        template = '\t' * layer.__name__.count('.') + 'input: %s: %s'
        try:
            shape = ipt[0].shape if isinstance(ipt, (tuple, list)) and len(ipt)>=1 else ipt.shape if len(ipt)!=0 else None
        except:
            shape = ipt
        print(template % (layer.__name__, shape))
    ...
    print(next(modules)[1])
    for n, l in modules:
        l.__name__ = n
        debug_hook.append(l.register_forward_hook(output_shape))
        debug_hook.append(l.register_forward_pre_hook(input_shape))
