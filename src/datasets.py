...
class BaseLightningDataModule(LightningDataModule):
    ...
    def add_argparse_args(cls, parser: ArgumentParser):
        ...
        parser.add_argument('--reset_data', action='store_true', help='whether to reset processed data saved on your desk.')
        parser.add_argument('--val_is_test', action='store_true', help='set test set as validation set to check the tendency of fscore.')
...
class Beer(BaseDataset):
    ...
    def setup(self):
        ...
        saved_file = self.get_save_path(f'{prev_tag.strip(".")}beer_aspect{self.aspect}_{self.part}.json')
        ...
        if self.part in ['train', 'heldout']:
            filename = os.path.join(self.data_path, '%sreviews.aspect%s.%s.txt' % (prev_tag, self.aspect, self.part))
            with open(filename, encoding='utf8') as f:
                for line in tqdm(f, desc=f'reading {self.part} set'):
                    ...
                    self.data.append((text, float(scores) if self.loss_fn=='mse' else int(scores), None, ids, token_offsets))

            if self.loss_fn == 'ce' and self.balance and self.part =='train':
                self.data = self.balance_data(self.data)

        else:
            filename = os.path.join(self.data_path, '%sannotations.json'%(prev_tag))
            with open(filename, encoding='utf8') as f:
                for line in tqdm(f, desc=f'reading {self.part} set'):
                    ...
                    scores = scores[self.aspect]
                    rationales = each['%d' % self.aspect]


                    self.data.append((text, float(scores) if self.loss_fn=='mse' else int(scores), rationales, ids, token_offsets))
...
class Hotel(BaseDataset):
    ...
    def setup(self):
        saved_file = self.get_save_path(f'hotel_aspect{self.aspect}_{self.part}.json')
        ...
        if self.part in ['train', 'dev']:
            ...
            with open(filename, encoding='utf8') as f:
                ...
                for line in tqdm(f, desc=f'reading {self.part} set'):
                    ...
                    self.data.append((text, int(scores), spans, ids, token_offsets))
...
class Eraser(BaseDataset):
    ...
    def setup(self):
        ...
        :return:
        '''
        saved_file = self.get_save_path(f'{self.part}_{self.max_len}_{self.max_query_len}_{self.sentence_level}.json')
        ...
        for data in tqdm(datas):
            ...
            if not rationales:
                rationales.append((-1, -1))
            self.data.append((text, int(self.label_map[data.classification]), rationales, query, sentence_spans, ids, query_ids, token_offsets))
...
class BaseCollator:
    ...
    def pad(self, x, length, pad_content):
        ...
        if len(x) > length:
            return x[:length]
        return x + [pad_content] * (length - len(x))
...
class Collator(BaseCollator):
    ...
    def __call__(self, batch):
        ...
        has_rationale = any([bool(each) for each in rationales])

        self.rationale_is_list = \
            has_rationale and (not bool(rationales[0]) or isinstance(rationales[0][0], (tuple, list)))
        # print(rationales)
            ...
            for idx, (rat, offset) in enumerate(zip(rationales, token_offsets)):
                ...
                for i, r in enumerate(rat):
                    if self.rationale_is_list:
                        if offset[r[0]] > self.max_len:
                            continue
                        cur_rationale.append((offset[r[0]], min(offset[r[1]], self.max_len)))
                    else:
                        cur_rationale+=[r]*(offset[i]-(0 if i==0 else offset[i-1]))
...
class CollatorWithQuery(BaseCollator):
    ...
    def __call__(self, batch):
        ...
        token_type_ids = []
        new_ids = []
        for idx, (txt, que, rat, ss, to) in enumerate(zip(ids, query_ids, rationales,
                                                      sentence_spans, token_offsets)):
            # 将query拼接到token之前。
            # query 和 text 之间有一个 <sep> .
            # 拼接之前展开子词
            txt=list(chain.from_iterable(txt))
            txt = que + [self.sep_id] + txt
        ...
        if self.sentence_level:
            ...
            padded_sentence_type_ids = self.find_and_pad_maxlen(sentence_type_ids, -1)
            # to tensor.
            sentence_mask = torch.tensor(padded_sentence_mask, dtype=torch.long)
        ...
        padded_tokens = self.find_and_pad_maxlen(tokens, self.pad_id)
        padded_rationales = self.find_and_pad_maxlen(rationales, (-1, -1))
        padded_sentence_spans = self.find_and_pad_maxlen(sentence_spans, (-1, -1))
        padded_token_type_ids = self.find_and_pad_maxlen(token_type_ids, -1)

        # to tensor.
        tensors = torch.tensor(padded_tokens, dtype=torch.long)
        mask = tensors != self.pad_id
        label = torch.tensor(label, dtype=torch.float32).unsqueeze(-1) # [B, 1]
        tensor_rationales = torch.tensor(padded_rationales, dtype=torch.long)
...
if __name__ == '__main__':
    ...
    collate_fn = CollatorWithQuery(vocab, max_len=1000000, sentence_level=False)
    for ds in datasets:
